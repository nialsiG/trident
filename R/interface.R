...
trident.gui <- function() {
  ...
  disable.cmd <- function() {
    ...
    tcltk::tkconfigure(MAPSUR.BTN, state = "disable")

    # Data
    tcltk::tkconfigure(OPEN.BTN, state = "disable")
    tcltk::tkconfigure(COMBINE.BTN, state = "disable")
    tcltk::tkconfigure(BUILD.BTN, state = "disable")
    tcltk::tkconfigure(REMOVE.BTN, state = "disable")
  ...
  reable.cmd <- function() {
    # Base
    tcltk::tkconfigure(LOAD.BTN, state = "normal")
    tcltk::tkconfigure(BATCH.BTN, state = "normal")
    tcltk::tkconfigure(OPEN.BTN, state = "normal")
    ...
    if (!is.null(DMTA$SURF)) {
      ...
      tcltk::tkconfigure(MAPSUR.BTN, state = "normal")
    }

    # is there a dataset?
    if (!is.null(PROJECT$DATASET)) {
      tcltk::tkconfigure(BUILD.BTN, state = "normal")
      tcltk::tkconfigure(REMOVE.BTN, state = "normal")
      ...
      tcltk::tkconfigure(COMBINE.BTN, state = "normal")
      # is there a factor?
  ...
  save.as.cmd <- function() {
    PROJECT$SAVENAME <<- tcltk::tclvalue(tcltk::tkgetSaveFile(title = "Save project...", initialfile = paste("Untitled", sep = "")))
    rlist::list.save(x = PROJECT, file = paste(PROJECT$SAVENAME, ".rds", sep = ""))
  ...
  save.table.cmd <- function(x) {
    File <- tcltk::tclvalue(tcltk::tkgetSaveFile(title = "Save table as...",
                                                 ...
                                                 filetypes = paste (
                                                   ...
                                                   "{{Excel files} {.xls}}",
                                                   "{{All files} {*}}", sep = " ")))
    if (utils::tail(unlist(strsplit(File, as.character("\\."))), 1) == "csv") readr::write_excel_csv(x, file = File)
    if (utils::tail(unlist(strsplit(File, as.character("\\."))), 1) == "xls") readr::write_excel_csv2(x, file = File)
    if (utils::tail(unlist(strsplit(File, as.character("\\."))), 1) == "txt") readr::write_delim(x, file = File, delim = " ")
  }
  # ...build.vartable.cmd----
  build.vartable.cmd <- function(x) { # where x is a data.frame object
    ...
    Results$Type[grep('Asfc2', Names, value = FALSE)] <- "complexity"
    ...
    Results$Type[grep('Smc1', Names, value = FALSE)] <- "topology"
    Results$Type[grep('Smc2', Names, value = FALSE)] <- "topology"
  ...
  # ...refresh.cmd----
  refresh.cmd <- function(na.rm = TRUE, var.0.rm = TRUE) {

    Factors  <- dplyr::select_if(PROJECT$DATASET, is.factor)
    Numerics <- dplyr::select_if(PROJECT$DATASET, is.numeric)

    if (!any(diag(var(Numerics, na.rm = TRUE)) == 0)) {
      if (!is.na(max(unlist(PROJECT$DATASET)))) {
        PROJECT$DATASET <<- dplyr::bind_cols(Factors, Numerics)
        PROJECT$VARIABLES <<- dplyr::bind_rows(PROJECT$VARIABLES[which(PROJECT$VARIABLES$Variables %in% colnames(Factors)), ],
                                               PROJECT$VARIABLES[which(PROJECT$VARIABLES$Variables %in% colnames(Numerics)), ])
      }
    }

    # ...Step 1: remove columns with only identical values?
    if (var.0.rm) {
      if (any(diag(var(Numerics, na.rm = TRUE)) == 0)) {
        Mylist <- colnames(Numerics)[diag(var(Numerics, na.rm = TRUE)) == 0]
        .TRIDENT$narm1234 <- tcltk::tktoplevel()
        tcltk::tkwm.title(.TRIDENT$narm1234, paste("trident", METADATA$VERSION, "- remove variables..."))
        tcltk2::tk2ico.setFromFile(.TRIDENT$narm1234, system.file("extdata","pics","trident.ico", package = "trident"))

        VARLIST1 <- tcltk2::tk2listbox(.TRIDENT$narm1234, values = Mylist, selectmode = "extended", height = 12, tip = "", scroll = "y", autoscroll = "x", enabled = TRUE)
        OK.BTN1 <- tcltk2::tk2button(.TRIDENT$narm1234, text = "Remove", command = function() {
          Myselect <- Mylist[tcltk2::selection(VARLIST1)]
          Newmerics <- Numerics[, which(!colnames(Numerics) %in% Myselect)]
          PROJECT$DATASET <<- dplyr::bind_cols(Factors, Newmerics)
          PROJECT$VARIABLES <<- dplyr::bind_rows(PROJECT$VARIABLES[which(PROJECT$VARIABLES$Variables %in% colnames(Factors)), ],
                                                 PROJECT$VARIABLES[which(PROJECT$VARIABLES$Variables %in% colnames(Newmerics)), ])
          refresh.cmd(na.rm = FALSE, var.0.rm = FALSE)
          tcltk::tkdestroy(.TRIDENT$narm1234)
        })
        # Ignore button
        CANCEL.BTN1 <- tcltk2::tk2button(.TRIDENT$narm1234, text = "Ignore", tip = "", command = function() tcltk::tkdestroy(.TRIDENT$narm1234))
        # grid all
        tcltk::tkgrid(tcltk::tklabel(.TRIDENT$narm1234,
        text = "The following variables contain
        only identical values, which might
        cause malfunction (ex. during top3,
        multichecks...)
        Do you want to remove them?"), columnspan = 2)
        tcltk::tkgrid(VARLIST1, columnspan = 2, sticky = "ew")
        tcltk::tkgrid(OK.BTN1, CANCEL.BTN1)
      }
    }
    ...
    if (na.rm) {
      if (is.na(max(unlist(PROJECT$DATASET)))) {
        ...
        .TRIDENT$narm5678 <- tcltk::tktoplevel()
        VARLIST2 <- tcltk2::tk2listbox(.TRIDENT$narm5678, values = Mylist, selectmode = "extended", height = 12, tip = "", scroll = "y", autoscroll = "x", enabled = TRUE)
        OK.BTN2 <- tcltk2::tk2button(.TRIDENT$narm5678, text = "Remove", command = function() {
          Myselect <- Mylist[tcltk2::selection(VARLIST2)]
          Newmerics <- Numerics[, which(!colnames(Numerics) %in% Myselect)]
          PROJECT$DATASET <<- dplyr::bind_cols(Factors, Newmerics)
          PROJECT$VARIABLES <<- dplyr::bind_rows(PROJECT$VARIABLES[which(PROJECT$VARIABLES$Variables %in% colnames(Factors)), ],
                                                 PROJECT$VARIABLES[which(PROJECT$VARIABLES$Variables %in% colnames(Newmerics)), ])
          refresh.cmd(na.rm = FALSE, var.0.rm = FALSE)
          tcltk::tkdestroy(.TRIDENT$narm5678)
        })
        # Ignore button
        CANCEL.BTN2 <- tcltk2::tk2button(.TRIDENT$narm5678, text = "Ignore", tip = "", command = function() tcltk::tkdestroy(.TRIDENT$narm5678))
        # grid all
        tcltk::tkgrid(tcltk::tklabel(.TRIDENT$narm5678, text = "The following variables contain NAs\nDo you want to remove variables?"), columnspan = 2)
        tcltk::tkgrid(VARLIST2)
        tcltk::tkgrid(OK.BTN2, CANCEL.BTN2)
      }
    }

    # define the width of the tables
  ...
  biplot.cmd <- function(df) {
    ...
    FACTORLIST <- tcltk2::tk2listbox(.TRIDENT$VarSelect1234,
                                     ...
                                     autoscroll = "x",
                                     enabled = TRUE)
    tcltk::tkselection.set(FACTORLIST, 1)
    ...
    OK.BTN <- tcltk2::tk2button(.TRIDENT$VarSelect1234, text = "OK", command = function() {
      ...
      Plot <- ggplot2::ggplot(data = Mydf, ggplot2::aes(x = Mydf[, Myx], y = Mydf[, Myy])) +
        ...
        ggplot2::theme(text = ggplot2::element_text(family = "serif"),
                       legend.text = ggplot2::element_text(colour = "black", size = 12, face = "bold"),
  ...
  plot.pca.cmd <- function(df1, df2) {
    ...
    Numerics <- dplyr::select_if(Mydf, is.numeric)
    Factors  <- dplyr::select_if(Mydf, is.factor)
    # A second data frame for sup vars
    Mydf2 <- df2
    Mydf2 <- Mydf2[!is.infinite(rowSums(dplyr::select_if(Mydf2, is.numeric))), ]
    Mydf2 <- stats::na.omit(Mydf2)
    Numerics2 <- dplyr::select_if(Mydf2, is.numeric)
    ...
    SUPVARLIST <- tcltk2::tk2listbox(.TRIDENT$VarSelect1234, values = colnames(Numerics2), selectmode = "extended", height = 12, tip = "", scroll = "y", autoscroll = "x", enabled = TRUE)
    FACTORLIST <- tcltk2::tk2listbox(.TRIDENT$VarSelect1234, values = colnames(Factors), selectmode = "single", height = 12, tip = "", scroll = "y", autoscroll = "x", enabled = TRUE)

    OK.BTN <- tcltk2::tk2button(.TRIDENT$VarSelect1234, text = "OK", command = function() {

      Myvars <<- colnames(Numerics)[tcltk2::selection(VARLIST)]
      Mysupvars <<- colnames(Numerics2)[tcltk2::selection(SUPVARLIST)]
      Myfactor <<- colnames(Factors)[tcltk2::selection(FACTORLIST)]
      Mypcadata <- dplyr::bind_cols(Mydf[, which(colnames(Mydf) %in% Myvars)], Mydf2[, which(colnames(Mydf2) %in% Mysupvars)])

      if (length(tcltk2::selection(SUPVARLIST)) == 0) {
        Mypca <- FactoMineR::PCA(Mypcadata,
                                 graph = FALSE,
      ...
      PLOT.BTN <- tcltk::tkbutton(.TRIDENT$PCSelect5678,
                                  ...
                                  command = function() {
                                    ...
                                    Plot <- factoextra::fviz_pca_ind(Mypca, axes = c(tcltk2::selection(MYPCLIST1), tcltk2::selection(MYPCLIST2)),
                                      ...
                                      ggplot2::scale_color_manual(name = Myfactor, labels = levels(Factors[, Myfactor]), values = PROJECT$OPTIONS$PLOT.COLORS) +
                                      ggplot2::scale_fill_manual(name = Myfactor, labels = levels(Factors[, Myfactor]), values = PROJECT$OPTIONS$PLOT.COLORS) +
                                      ggplot2::scale_shape_manual(name = Myfactor, labels = levels(Factors[, Myfactor]), values = PROJECT$OPTIONS$PLOT.PCH) +
                                      ggplot2::guides(colour = ggplot2::guide_legend(override.aes = list(size = 2))) +
      ...
      PLOT.COR.BTN <- tcltk::tkbutton(.TRIDENT$PCSelect5678,
                                      ...
                                      command = function() {
                                        ...
                                        Plot2 <- factoextra::fviz_pca_var(Mypca,
                                                                          axes = c(tcltk2::selection(MYPCLIST1), tcltk2::selection(MYPCLIST2)),
                                                                          repel = TRUE,
                                                                          col.var = "steelblue",
      ...
      ADD.SUP.BTN <- tcltk::tkbutton(.TRIDENT$PCSelect5678,
                                     ...
                                     command = function() {
                                       ...
                                       if (tools::file_ext(File.tmp) == "txt") {
                                         Mytable <- data.frame(stringsAsFactors = TRUE,
                                                               utils::read.table(file = File.tmp,
                                                                                 header = TRUE,
                                                                                 sep = "",
                                                                                 dec = ".",
                                                                                 row.names = NULL,
                                       ...
                                       # Match columns
                                       Mypcasupdata <- data.frame(Mytable[, c(which(colnames(Mytable) %in% Myvars), which(colnames(Mytable) %in% Mysupvars))])
                                       if (length(Mypcasupdata[1, ]) != length (Mypcadata[1, ])) {
                                       ...
                                       # New PCA
                                       Mynewpcadata <- dplyr::union(Mypcadata, Mypcasupdata)

                                       if (length(Mysupvars) > 0) {
                                         Mypca <<- FactoMineR::PCA(Mynewpcadata,
                                                                   graph = FALSE,
                                                                   ind.sup = c((length(Mypcadata[, 1]) + 1):(length(Mypcadata[, 1]) + length(Mypcasupdata[, 1]))),
                                                                   quanti.sup = c((length(Myvars) + 1):(length(Myvars) + length(Mysupvars))),
                                                                   ncp = ncol(length(Myvars)))
                                       } else {
                                         Mypca <<- FactoMineR::PCA(Mynewpcadata,
  ...
  # TKGUI - MAIN WINDOW----
  # Prepare environment
  .TRIDENT <<- new.env(parent = emptyenv())
  ...
  # ...Manual
  tcltk::tkadd(MENU$HELP, "command", label = "Manual", command = function() {})

  # ...About
  tcltk::tkadd(MENU$HELP, "separator")
  tcltk::tkadd(MENU$HELP, "command", label = "About", command = function(){
    .TRIDENT$ABOUT <- tcltk::tktoplevel()
    tcltk2::tk2ico.setFromFile(.TRIDENT$ABOUT, system.file("extdata","pics","trident.ico", package = "trident"))
    tcltk::tkwm.title(.TRIDENT$ABOUT, paste("trident", METADATA$VERSION, "- About"))
    tcltk::tkgrid(tcltk::tklabel(.TRIDENT$ABOUT, image = tcltk::tkimage.create("photo", file = system.file("extdata","pics","trident.gif", package = "trident"))),
                  tcltk::tklabel(.TRIDENT$ABOUT, text = paste("TRIDENT", METADATA$VERSION)),
                  sticky = "ew")
    tcltk::tkgrid(tcltk::tklabel(.TRIDENT$ABOUT, text = "package 'trident': A library for dental microwear
    ...
    # ...Help files
    tcltk::tkadd(MENU$HELP, "separator")
    tcltk::tkadd(MENU$HELP, "command", label = "Francisco et al. (2018) Surf Topogr Metrol Prop", command = function() utils::browseURL("https://hal.archives-ouvertes.fr/hal-03042216"))
    tcltk::tkadd(MENU$HELP, "command", label = "Francisco et al. (2018) Technologies", command = function() utils::browseURL("https://hal.archives-ouvertes.fr/hal-03042226"))
    tcltk::tkadd(MENU$HELP, "command", label = "Merceron et al. (2017) Biosurf Biotribol", command = function() utils::browseURL("https://hal.archives-ouvertes.fr/hal-02100359"))
    ...
    tcltk::tkadd(COMPUTE.MENU, "command", label = "Complexity", command = function() {
      ...
      DMTA$COMPUT$COMPLEX <<- Complexity[3:4]
      Mytable <- data.frame(File = Complexity[1],
                            Asfc2 = Complexity[3],
                            stringsAsFactors = TRUE)
      Myvartable <- data.frame(Variables = colnames(Mytable), Type = c("integer", rep(paste(unlist(Complexity[2])), 1)))
    ...
    tcltk::tkadd(COMPUTE.MENU, "command", label = "Compute all", command = function() {
      ...
      Mytable <- data.frame(File = Complexity[1],
                            ...
                            Snb2 = Topo[8],
                            Sh = Topo[9], stringsAsFactors = TRUE)
      Myvartable <- data.frame(Variables = colnames(Mytable), Type = c("integer", rep(paste(unlist(Complexity[2])), 1), rep(paste(unlist(Elev[2])), 9), rep(paste(unlist(Spat[2])), 7), rep(paste(unlist(Topo[2])), 7)))
      if (!is.null(DMTA$DATASET)) DMTA$DATASET <<- merge(DMTA$DATASET, Mytable, by = intersect(names(DMTA$DATASET), names(Mytable)), all = TRUE, sort = FALSE, no.dups = TRUE)
      if (is.null(DMTA$DATASET)) DMTA$DATASET <<- Mytable
      if (!is.null(DMTA$VARIABLES))  DMTA$VARIABLES <<- dplyr::union(DMTA$VARIABLES, Myvartable)
      if (is.null(DMTA$VARIABLES)) DMTA$VARIABLES <<- Myvartable
    ...
    tcltk::tkadd(HETERO.MENU, "command", label = "Complexity", command = function() {
      ...
      OK.BTN2 <- tcltk2::tk2button(.TRIDENT$size1234, text = "Ok", tip = "", command = function () {
        ...
        DMTA$COMPUT.HTR$COMPLEX <<- data.frame(Complexity[3:4])
        ...
        Myvartable <- data.frame(Variables = colnames(Mytable), Type = c("integer", rep(paste(unlist(Complexity[2])), 2*15)))
        if (!is.null(DMTA$DATASET)) DMTA$DATASET <<- merge(DMTA$DATASET, Mytable, by = intersect(names(DMTA$DATASET), names(Mytable)), all = TRUE, sort = FALSE, no.dups = TRUE)
        if (is.null(DMTA$DATASET)) DMTA$DATASET <<- Mytable
        if (!is.null(DMTA$VARIABLES))  DMTA$VARIABLES <<- dplyr::union(DMTA$VARIABLES, Myvartable)
        if (is.null(DMTA$VARIABLES)) DMTA$VARIABLES <<- Myvartable
    ...
    tcltk::tkadd(HETERO.MENU, "command", label = "Topology parameters", command = function() {
      ...
      OK.BTN2 <- tcltk2::tk2button(.TRIDENT$size1234, text = "Ok", tip = "", command = function () {
        ...
        # ...Computation
        Topo <- trident::dmta.topology(sur = DMTA$SURF, type = "multi", size.x = Size.x, size.y = Size.y, size.n = Size.n)
        DMTA$COMPUT.HTR$TOPO <<- data.frame(Topo[3:9])
        Mytable <- data.frame(File = Topo[1], c(plyr::colwise(trident::trident.hetero)(data.frame(DMTA$COMPUT.HTR$TOPO))), stringsAsFactors = TRUE)
        Myvartable <- data.frame(Variables = colnames(Mytable), Type = c("integer", rep(paste(unlist(Topo[2])), 7*15)))
        if (!is.null(DMTA$DATASET)) DMTA$DATASET <<- merge(DMTA$DATASET, Mytable, by = intersect(names(DMTA$DATASET), names(Mytable)), all = TRUE, sort = FALSE, no.dups = TRUE)
        if (is.null(DMTA$DATASET)) DMTA$DATASET <<- Mytable
        if (!is.null(DMTA$VARIABLES))  DMTA$VARIABLES <<- dplyr::union(DMTA$VARIABLES, Myvartable)
        if (is.null(DMTA$VARIABLES)) DMTA$VARIABLES <<- Myvartable
    ...
    tcltk::tkadd(HETERO.MENU, "command", label = "Compute all", command = function() {
      ...
      OK.BTN2 <- tcltk2::tk2button(.TRIDENT$size1234, text = "Ok", tip = "", command = function () {
        ...
        Mytable <- dplyr::bind_cols(Mytable, data.frame(c(plyr::colwise(trident::trident.hetero)(data.frame(DMTA$COMPUT.HTR$TOPO)))))
        Myvartable <- data.frame(Variables = colnames(Mytable), Type = c("integer", rep(paste(unlist(Complexity[2])), 2*15), rep(paste(unlist(Elev[2])), 9*15), rep(paste(unlist(Spat[2])), 7*15), rep(paste(unlist(Topo[2])), 7*15)))
        if (!is.null(DMTA$DATASET)) DMTA$DATASET <<- merge(DMTA$DATASET, Mytable, by = intersect(names(DMTA$DATASET), names(Mytable)), all = TRUE, sort = FALSE, no.dups = TRUE)
        if (is.null(DMTA$DATASET)) DMTA$DATASET <<- Mytable
        if (!is.null(DMTA$VARIABLES))  DMTA$VARIABLES <<- dplyr::union(DMTA$VARIABLES, Myvartable)
        if (is.null(DMTA$VARIABLES)) DMTA$VARIABLES <<- Myvartable
    ...
    HISTO.BTN <- tcltk::tkbutton(NOTEBOOK$MICRO, text = "Histograms", image = tcltk::tkimage.create("photo", file = system.file("extdata","pics","histo.gif", package = "trident")), height = 50, relief = "flat", compound = "top", command = function(){
      ...
      tcltk2::tk2ico.setFromFile(.TRIDENT$VarSelect1234, system.file("extdata","pics","trident.ico", package = "trident"))
      Mydf <- dplyr::bind_cols(DMTA$COMPUT.HTR$COMPLEX, DMTA$COMPUT.HTR$ELEV, DMTA$COMPUT.HTR$SPAT, DMTA$COMPUT.HTR$TOPO)
      ...
      OK.BTN <- tcltk2::tk2button(.TRIDENT$VarSelect1234, text = "OK", command = function() {
        ...
        Plot <- ggplot2::ggplot(data = Mydf, ggplot2::aes(x = Mydf[, Myx])) +
          ...
          ggplot2::theme(text = ggplot2::element_text(family = "serif"), legend.text = ggplot2::element_text(colour = "black", size = 12, face = "bold"),
                         ...
                         panel.grid.minor = ggplot2::element_blank(),
                         panel.ontop = FALSE,
                         axis.title.x = ggplot2::element_text(size = 12, angle = 00, face = "italic"),
                         axis.title.y = ggplot2::element_text(size = 12, angle = 90, face = "italic")) +
          ggplot2::guides(colour = ggplot2::guide_legend(override.aes = list(size = 2)))
    ...
    MAPSUR.BTN <- tcltk::tkbutton(NOTEBOOK$MICRO, text = "3d map", image = tcltk::tkimage.create("photo", file = system.file("extdata","pics","sur.gif", package = "trident")), height = 50, relief = "flat", compound = "top", command = function(){
      ...
      # change height's unit to micrometers:
      Mydf$z <- Mydf$z*1000
      ...
      COL.LEVELS.SPNBX <- tcltk2::tk2spinbox(.TRIDENT$VarSelect1234, textvariable = DMTA$LEVELS <<- tcltk::tclVar(256), from = 2, to = 256, increment = 1)

      D3MAP.BTN <- tcltk2::tk2button(.TRIDENT$VarSelect1234, text = "3D map", command = function() {
      ...
      D2MAP.BTN <- tcltk2::tk2button(.TRIDENT$VarSelect1234, text = "2D map", command = function() {
        col.levels <- as.numeric(tcltk::tclvalue(tcltk::tkget(COL.LEVELS.SPNBX)))
        if (tcltk::tclvalue(tcltk::tkget(COL.CBBX)) == "Grayscale") col.range <- c("black","white")
        if (tcltk::tclvalue(tcltk::tkget(COL.CBBX)) == "Temperature") col.range <- c("royalblue4","royalblue","lightseagreen","olivedrab3","yellow1","orange","orangered","red","firebrick4")
        if (tcltk::tclvalue(tcltk::tkget(COL.CBBX)) == "Colorblind") col.range <- c("lightgreen","goldenrod1","yellow1","white","lightskyblue","dodgerblue4","royalblue")
      ...
      SAVE.BTN <- tcltk2::tk2button(.TRIDENT$VarSelect1234, text = "Save 3D map", command = function() {
        rgl::snapshot3d(file = tcltk::tclvalue(tcltk::tkgetSaveFile(parent = .TRIDENT$VarSelect1234, title = "Save 3D map as...", initialfile = "untitled", defaultextension = ".png")), fmt = "png")
      })
      ...
      tcltk::tkgrid(D3MAP.BTN, D2MAP.BTN, sticky = "ew", padx = 5, pady = 0)
    ...
    BATCH.BTN <- tcltk::tkbutton(NOTEBOOK$MICRO, text = "Batch analysis", image = tcltk::tkimage.create("photo", file = system.file("extdata","pics","batch.gif", package = "trident")), height = 50, relief = "flat", compound = "top", command = function(){
      ...
      TOPO.H.CHKBTN <- tcltk2::tk2checkbutton(.TRIDENT$BatchDmta1234, text = "Topology heterogeneity")
      ...
      tcltk::tkconfigure(TOPO.H.CHKBTN, variable = PROJECT$OPTIONS$BATCH.TOPO.H.VALUE)
      ...
      OK.BTN <- tcltk2::tk2button(.TRIDENT$BatchDmta1234, text = "OK", command = function() {
        ...
        OK.BTN2 <- tcltk2::tk2button(.TRIDENT$size1234, text = "Ok", tip = "", command = function () {
          ...
          Batch.fun <- function(sur, tasklist, removal, do.std) {
            ...
            if (tasklist[8] == "1") {
              HTopo <- trident::dmta.topology(sur = sur, type = "multi", size.x = Size.x, size.y = Size.y, size.n = Size.n)
              Mytable <- data.frame(Mytable, c(plyr::colwise(trident::trident.hetero)(data.frame(HTopo[3:9]))))
            }
            return(Mytable)
          }
          ...
          Mytasklist <- c(tcltk::tclvalue(PROJECT$OPTIONS$BATCH.COMPLEX.VALUE),
                          ...
                          tcltk::tclvalue(PROJECT$OPTIONS$BATCH.TOPO.H.VALUE))
      ...
      tcltk::tkgrid(TOPO.CHKBTN, TOPO.H.CHKBTN, sticky = "w")
      tcltk::tkgrid(STD.CHKBTN, sticky = "w")
    ...
    tcltk::tkadd(TRANS.MENU, "command", label = "Boxcox transformation", command = function(){
      ...
      OK.BTN <- tcltk2::tk2button(.TRIDENT$VarSelect1234, text = "OK", command = function(){
        ...
        if(length(levels(Factors[, Myfactor])) < 2) {
          tcltk::tkmessageBox(text = "Selected factor should have at least 2 levels")
          stop('trident.boxcox can be applied only if y is a factor with 2 or more levels')
        }
    ...
    ADD.VAR.BTN <- tcltk::tkbutton(NOTEBOOK$VARIA, text = "Add\nfactor", image = tcltk::tkimage.create("photo", file = system.file("extdata","pics","addvar.gif", package = "trident")), height = 50, relief = "flat", compound = "top", command = function(){
      ...
      AUTOMATIC.BTN <- tcltk2::tk2button(.TRIDENT$ModeSelect1234, text = "Automatic...", tip = "Extract factor from file name", command = function() {
        ...
        tcltk::tkselection.set(FACTOR.LISTBOX, 0)
    ...
    SUM.BTN <- tcltk::tkbutton(NOTEBOOK$VARIA, text = "Summary", image = tcltk::tkimage.create("photo", file = system.file("extdata","pics","summary.gif", package = "trident")), height = 50, relief = "flat", compound = "top", command = function(){
      ...
      tcltk::tkselection.set(FACTORLIST, 1)
      OK.BTN <- tcltk2::tk2button(.TRIDENT$VarSelect1234, text = "OK", command = function(){
        Myfactor <- Factors[, tcltk2::selection(FACTORLIST)]
        tcltk::tkdestroy(.TRIDENT$VarSelect1234)
      ...
      tcltk::tkgrid(FACTORLIST, columnspan = 2)
      tcltk::tkgrid(OK.BTN, CANCEL.BTN)
      tcltk::tcl("wm", "attributes", .TRIDENT$VarSelect1234, topmost = TRUE)
      tcltk::tcl("wm", "attributes", .TRIDENT$VarSelect1234, topmost = FALSE)
    })
      ...
      OK.BTN <- tcltk2::tk2button(.TRIDENT$VarSelect1234, text = "OK", command = function(){
        ...
        if (length(Mycheck$variable[which(Mycheck$is.discriminant == TRUE)]) != 0) {
          ...
          PCA.BTN <- tcltk2::tk2button(.TRIDENT$TableWin1234$BUTTONS, text = "PCA", tip = "", command = function() {
            Mydf <- data.frame(Factors, Numerics)
            plot.pca.cmd(df1 = Mydf, df2 = PROJECT$DATASET)
          })
    ...
    MULTI.BTN <- tcltk::tkbutton(NOTEBOOK$VARIA, image = tcltk::tkimage.create("photo", file = system.file("extdata","pics","multicheck.gif", package = "trident")), height = 50, relief = "flat",
                                 text = "Multicheck", compound = "top", command = function(){
                                   ...
                                   OK.BTN <- tcltk2::tk2button(.TRIDENT$VarSelect1234, text = "OK", command = function(){
                                     ...
                                     # destroy the waiting message, re-able
                                     tcltk::tkdestroy(.TRIDENT$busy)
                                     reable.cmd()
    ...
    PCA.BTN <- tcltk::tkbutton(NOTEBOOK$PLOTS, image = tcltk::tkimage.create("photo", file = system.file("extdata","pics","pca.gif", package = "trident")), height = 50, relief = "flat",
                               text = "PCA", compound = "top", command = function() {
                                 plot.pca.cmd(df1 = PROJECT$DATASET, df2 = PROJECT$DATASET)
                               })
